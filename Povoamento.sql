...
INSERT INTO Dim_Urgency_Prescription(Cod_Prescription,Prof_Prescription,FK_Date_Prescription)  -- done
...
select count(*) FROM `dados`.`urgency_prescriptions` WHERE `COD_PRESCRIPTION`>=16694000 AND `COD_PRESCRIPTION` <=17235985;
...
-- Povoar Prescription_Drug
DELIMITER //
CREATE DEFINER=`root`@`localhost` PROCEDURE `insert_Prescription_Drug`(bound VARCHAR(255))
BEGIN
DECLARE id INT;
DECLARE value TEXT;
...
DECLARE cur1 CURSOR FOR SELECT distinct uP.COD_PRESCRIPTION, uP.cast
                                     FROM `dados`.`urgency_prescriptions` uP; -- t1
